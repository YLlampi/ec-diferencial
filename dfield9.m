...
%*****************************************************************************80
%
%% dfield() is an interactive tool for studying differential equations.  
%
%  Discussion:
%
%    When dfield9 is executed, a dfield9 Setup
...
%    self explanatory.  The Settings option allows the user
...
%
  ...
  if strcmp(action,'initialize')
    ...
    set(dfset,'position',[30 dfigurebot dfigwidth dfigureheight]);
    
    set(frame(1),'position',defrwind);
    ...
    set(eq(1),'position',[eqlableft eqlabelbot lablen texth]);
    tcolor = get(gcf,'defaultuicontrolbackgroundcolor');
    ecolor = 'w';
    
    ud.h.xname=uicontrol('position',[eqleft, derbot, varw, texth],...
			 'style','edit',...
    ...
    eq(2) = uicontrol('style','text',...
		      'position',[eqleft+varw derbot-fudge equalw texth],...
		      'horizontalalignment','center','string',''' = ','visible','off',...
		      'backgroundcolor',tcolor);
    
    ud.h.der=uicontrol('position',[eqleft+varw+equalw derbot eqlength texth],...
		       'string',ud.o.der,...
		       'horizontalalignment','left','style','edit',...
    ...
    tnw = ext(3)+10;
    tvarl = eqleft + tnw;
    set(eq(3),'position',[eqleft,tbot-fudge,tnw,texth]);
    
    ud.h.tname = uicontrol('position',[tvarl tbot varw texth],...
			   'style','edit',...
    ...
    pframe = uicontrol('style','frame','position',pfrwind,'visible','on');
	...
	ud.h.pname(K) = uicontrol('style','edit',...
				  'position',[pleft pbot(jj) pnamew texth],...
				  'horizontalalignment','right','string',name,...
				  'userdata',K,...
				  'Callback',pncall,...
				  'visible','off',...
				  'backgroundcolor','w');
	equal(K) = uicontrol('style','text',...
			     'position',[peqleft pbot(jj)-fudge peqw texth],...
			     'horizontalalignment','center',...
			     'string','=',...
			     'visible','off');
	
	ud.h.pval(K) = uicontrol('style','edit',...
				 'position',[pvleft pbot(jj) pvalw texth],...
				 'string',value,...
				 'Callback',pvcall,...
				 'visible','off',...
				 'userdata',K,...
    ...
    frame(2) = uicontrol('style','frame','position',disfrwind,'visible','off');
    ...
    dwind = uicontrol('style','text',...
		      'position',[eqleft winbot1 disfrw-10 texth],...
		      'horizontalalignment','center',...
    ...
    set(get(mefile,'children'),'visible','off');
  ...
  meproceed = uimenu(mefile,'label','Proceed','Callback',...
    ...
    sysmenu = uimenu('label','Gallery','visible','off','position',3);
...
% 			'drawmode','fast',...	
	...
	dbutt(1) = uicontrol('style','push',...
			     'position',[buttl,dfaxb+2*buttsep,buttw,butth],...
			     'string','Stop',...
			     'Callback',stopstr,...
			     'visible','off',...
			     'tag','stop');
	...
	dbutt(2) = uicontrol('style','push',...
			     'position',[buttl,dfaxb,buttw,butth],...
			     'string','Quit',...
			     'Callback','dfield9(''quit'')',...
			     'visible','off');
	
	dbutt(3) = uicontrol('style','push',...
			     'position',[buttl,dfaxb+buttsep,buttw,butth],...
			     'string','Print',...
	...
	set(findobj(fmenu,'label','&Export...'),...
	    'position',3);
	set(findobj(fmenu,'label','Pa&ge Setup...'),'position',4,...
			  'separator','on');
	set(findobj(fmenu,'label','Print Pre&view...'),'position',5);
	set(findobj(fmenu,'label','Print Set&up...'),'position',6);
	set(findobj(fmenu,'label','&Print...'),'position',7);
	merestart = uimenu(fmenu,'label','Restart dfield9','Callback',...
			   'dfield9(''restart'')','separator','on','position',8);
	...
	set(findobj(emenu,'label','&Undo'),'separator','on',...
			  'position',8);
	set(findobj(emenu,'label','Cu&t'),'position',9);
	set(findobj(emenu,'label','&Copy'),'position',10);
	set(findobj(emenu,'label','&Paste'),'position',11);
	set(findobj(emenu,'label','&Select All'),'position',12);
	set(findobj(emenu,'label','Copy &Figure'),'position',13);
	set(findobj(emenu,'label','Copy &Options'),'position',14);
	set(findobj(emenu,'label','F&igure Properties'),'position',15);
	set(findobj(emenu,'label','&Axes Properties'),'position',16);
	set(findobj(emenu,'label','C&urrent Object Properties'),'position',17);
	
	% Options menu
	
	menu(1) = uimenu('label','Options','visible','off','position',3);
	...
	plcall = [
	    ...
	    '  ud.pline = ''on'';',...
	    '  set(ud.plineh,''visible'',''on'');',...	  
	    '  set(gcf,''userdata'',ud);',...
	    'else,',...
	    '  set(me,''label'',''Show the phase line.'');',...
	    '  ud.pline = ''off'';',...
	    '  set(ud.plineh,''visible'',''off'');',...	  
	...
	dud.solver(2) = uimenu(mesolve,'label','Euler',...
			       'checked',eulch,...
			       'Callback',solverstr,'userdata',eulset);
	
	dud.solver(3) = uimenu(mesolve,'label','Runge-Kutta 2',...
			       'checked',rk2ch,...
			       'Callback',solverstr,'userdata',rk2set);
	...
	set(findobj(gcf,'label','&View'),'position',4);
	...
	ppkbd = findobj('name','dfield9 Keyboard input','visible','on');
  ...
  elseif strcmp(action,'dirfield')
	...
	dud.plineh = plot([aa,aa],Dxint,'color',dud.color.pline);
      ...
      set([dud.plhand;dud.plineh],'visible',dud.pline);  
  ...
  elseif strcmp(action,'solution')
	...
	initpt = initpt([1:2]);
      ...
      set(stopbutt,'visible','on','enable','on');
  ...
  elseif strcmp(action,'kbd')
    ...
    kbd.inst = uicontrol('style','text', ...
                         'horizontalalignment','center',...
			 'string','Enter the initial conditions:');
    
    kbd.xname = uicontrol('style','text',...
			  'horizontalalignment','right','string',xnstr);
    ...
    kbd.spec = uicontrol('style','check', ... 
                         'horizontalalignment','center',...
    ...
    set(ppkbd,'position',figwind);
    set(kbd.fr1,'position',fr1wind);
    set(kbd.fr2,'position',fr2wind);
    set(kbd.inst,'position',instwind);
    set(kbd.xname,'position',xnwind);
    set(kbd.tname,'position',tnwind);
    set(kbd.xval,'position',xvwind);
    set(kbd.tval,'position',tvwind);
    set(kbd.comp,'position',compwind);
    set(kbd.close,'position',clwind);
    set(kbd.tname,'position',tnwind);
    set(kbd.tval,'position',tvwind);
    set(kbd.spec,'position',specwind);
    set(kbd.t0,'position',t0wind);
    set(kbd.t,'position',twind);
    set(kbd.tf,'position',tfwind);
    speccall = [
	'ud = get(gcf,''userdata'');',...
	'if get(gcbo,''value''),',...
    ...
    set(ppkbd,'userdata',kbd,'visible','on');
  ...
  elseif strcmp(action,'solvset')
    ...
    d1 = uicontrol('style','text','horizontalalignment','left',...
		   'string',d1string);
    ...
    ss = uicontrol('style','text','horizontalalignment','center',...
		   'string',ssname);
    ...
    set(setfig,'position',[20,figb,figw,figh]);
    set(setspfr,'position',[left,setspfrb,frw,setspfrht]);
    set(data.speed,'position',[sl,spb1,ssw,ht],...
		   'backgroundcolor',0.6*[1 1 1],...
		   'foregroundcolor','r');
    set(data.sp.min,'position',[sptl1,spb2,sptw,ht]);
    set(data.sp.max,'position',[sptl3,spb2,sptw,ht]);
    set(data.sp.val,'position',[sptl2,spb2,sptw,ht]);
    set(pps,'position',[sl,spb3,ssw,ht]);
    set(setfr,'position',[left,setfrb,frw,setfrh]);
    set(ss,'position',[sl,ssb,ssw,ht]);
    set(d1,'position',[d1l,d1b,stw,ht]);
    set(data.d1,'position',[d1l+stw,d1b,varl,ht]);
    set(gob,'position',[left,gobb,frw,ht]);
    set(data.chb,'position',[left,chbb,frw,ht]);
    if strcmp(name,'Dormand-Prince')
      rtol = uicontrol('style','text','horizontalalignment','left',...
		       'position',[sl+5,rtolb,stw-5,ht],...
		       'string','Relative error tolerance: '); 
      data.rtol = uicontrol('style','edit','Callback',rtolcall,...
			    'position',[sl+stw,rtolb,varl,ht],...
			    'string',num2str(data.settings.tol),...
			    'background','w');
      hmax = uicontrol('style','text','horizontalalignment','left',...
		       'position',[sl+5,hmaxb,stw-5,ht],...
		       'string','Maximum step size: '); 
      data.hmax = uicontrol('style','edit','Callback',hmaxcall,...
			    'position',[sl+stw,hmaxb,varl,ht],...
			    'string',num2str(data.settings.hmax),...
			    'background','w');
      set(d1,'position',[sl+5,d1b,stw-5,ht]);
      set(data.d1,'position',[sl+stw,d1b,varl,ht]);
    end
    
    set(setfig,'userdata',data);
    set(setfig,'units','normal');
    set(findobj(setfig,'type','uicontrol'),'units','normal');
    set(setfig,'visible','on','resize','on');
    set(get(setfig,'child'),'visible','on');
  ...
  elseif strcmp(action,'settings')
    ...
    cw1 = uicontrol('style','text','horizontalalignment','left',...
		    'string','The calculation window is');
    data.mag = uicontrol('style','edit','Callback',magcall,...
			 'string',num2str(kk),'background','w');
    cw2 = uicontrol('style','text','horizontalalignment','left',...
		    'string',' times as large as the ');
    cw3 = uicontrol('style','text','horizontalalignment','left',...
    ...
    set(setfig,'position',[20,figb,figw,figh]);
    set(dirffr,'position',[left,dirffrb,frw,dirffrh]);
    set(cwfr,'position',[left,cwfrb,frw,cwfrh]);
    set(ss,'position',[sl,ssb,cww,ht]);
    set(cw1,'position',[sl,cw2b,cw1w,ht]);
    set(cw2,'position',[sl+cw1w+cwew,cw2b,cw2w,ht]);
    set(cw3,'position',[sl,cw3b,cww,ht]);
    set(data.mag,'position',[sl+cw1w,cw2b,cwew,ht]);
    set(gob,'position',[gobl,bb,buttw,ht]);
    set(chb,'position',[chbl,bb,buttw,ht]);
    ...
    typeframe = uicontrol('style','frame','position',typewind,'visible','off');
    
    textframe = uicontrol('style','frame','position',textwind,'visible','off');
    ...
    nfptstext = uicontrol('style','text',...
			  'position',[textleft rb2b textw-2*nudge 1.5*ht],...
			  'string',nfptsstr,...
			  'horizon','center',...
			  'visible','off');
    ...
    npts = uicontrol('style','edit',...
		     'position',[textleft+(textw -30*ht/19)/2,rb1b 30*ht/19,ht],...
		     'string',npts,...
		     'Callback',callnfpts,...
		     'visible','off',...
		     'background','w');
  ...
  elseif strcmp(action,'setchange')
    ...
    elseif ~strcmp(dud.fieldtype,data.fieldtype)
      ...
      switch dud.fieldtype
       case 'lines'
	set(dud.arr.lines,'visible','on');
	set(dud.arr.arrows,'visible','off');
       case 'arrows'
	set(dud.arr.lines,'visible','off');
	set(dud.arr.arrows,'visible','on');
       case 'none'
	set(dud.arr.lines,'visible','off');
	set(dud.arr.arrows,'visible','off');
      end
  ...
  elseif strcmp(action,'delete')
    ...
    dud = get(disph,'userdata');
    arr = dud.arr;
    lv = get(arr.lines,'visible');
    av = get(arr.arrows,'visible');
    pv = get(dud.plineh,'visible');  
    ...
    set(arr.lines,'visible',lv);
    set(arr.arrows,'visible',av);
    set(dud.plineh,'visible',pv);  
    set(disph,'userdata',dud);
    set(disph,'WindowButtonDownFcn','dfield9(''down'')');
  ...
  elseif strcmp(action,'level')
    ...
    lev.inst1 = uicontrol('style','text','horizontalalignment','left',...
			  'string',inst1str);
    
    lev.lfcn = uicontrol('style','edit','horizontalalignment','center',...
			 'string',lfcn,'Callback','',...
			 'background','w');
    
    lev.ch(3) = uicontrol('style','radio','horizontalalignment','center',...
			  'min',0,'max',3,...
			  'value',0,...
			  'visible','on',...
			  'string','Let dfield9 decide.');
    
    lev.ch(2) = uicontrol('style','radio','horizontalalignment','center',...
			  'min',0,'max',2,...
			  'value',0,...
			  'string','Select a point in the Display Window.');
    
    lev.inst2 = uicontrol('style','text','horizontalalignment','left',...
			  'string',['Choose one of the following ways to',...
		          ' choose level value(s):']);
    
    lev.ch(1)  = uicontrol('style','radio','horizontalalignment','center',...
			   'min',0,'max',1,...
			   'value',0,...
			   'string','Enter a vector of level values.');
    
    lev.rhs = uicontrol('style','edit','horizontalalignment','center',...
			'string',' ','Callback','');
    ...
    set(dflevel,'position',figwind);
    set(lev.fr1,'position',fr1wind);
    set(lev.fr2,'position',fr2wind);
    set(lev.inst1,'position',inst1wind);
    set(lev.inst2,'position',inst2wind);
    set(lev.ch(1),'position',ch1wind);
    set(lev.ch(2),'position',ch2wind);
    set(lev.ch(3),'position',ch3wind);
    set(lev.rhs,'position',vwind);
    set(lev.lfcn,'position',fwind);
    set(lev.proc,'position',procwind);
    set(lev.close,'position',clwind);
    set(lev.ch(3),'value',3);
    set(lev.rhs,'enable','off');  
    
    child = get(dflevel,'children');
    set(dflevel,'visible','on','userdata',lev);
  ...
  elseif strcmp(action,'cdisp')
    ...
    fpos = get(dfdisp,'position');
    dfax = dud.axes;
    xd = get(dfax,'xlim');
    yd = get(dfax,'ylim');
    apos = get(dfax,'position');
  ...
  elseif strcmp(action,'loadsyst')  % This loads either a system or a gallery.
    ...
    if strcmp(type,'default')
      set(waith,'name','Loading the default gallery.','visible','on');
	...
	return;
      ...
      set(waith,'name',['Loading ',fname],'visible','on');
...
%        is described in Chapters 5 and 6 of John Dormand's book, 
